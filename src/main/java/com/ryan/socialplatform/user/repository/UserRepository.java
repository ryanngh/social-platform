...
public interface UserRepository extends JpaRepository<User, UUID> {
    ...
    @Query("""
                SELECT new com.ryan.socialplatform.user.dto.UserResponse(
                    u.id, p.firstName, p.lastName, p.avatarUrl, p.bannerUrl, p.bio,
                    p.pronouns, p.location, p.websiteUrl, p.birthday, p.pronunciation,
                    u.status, u.verified, u.createdAt, u.updatedAt
                )
                FROM User u
                LEFT JOIN UserProfile p ON p.userId = u.id
                WHERE u.id = :userId
            """)
    Optional<UserResponse> findProfileById(@Param("userId") UUID userId);
